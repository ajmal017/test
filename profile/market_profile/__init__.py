...
class MarketProfile(object):
    ...
    def __getitem__(self, index):
        if isinstance(index, slice): #Check if provided index is of type SLICE
            return MarketProfileSlice(self, slice(index.start, index.stop))
        elif isinstance(index, int):
            return MarketProfileMask(self, index)
        else:
...
class MarketProfileMask(object):
    def __init__(self, market_profile, mask):
        self.mp = market_profile
        self.ds = self.mp.df
        self.build_profile()
    ...
    def build_profile(self):
        #rounded_set = self.ds['Close'].apply(lambda x: self.mp.round_to_row(x))
        rounded_set = self.ds['High'].apply(lambda  x: self.mp.round_to_row(x))
