...
def strangle_payoff(dataFrame, strike_list, *args):
    """
    ...
    def price(list):
        ...
        dataFrame['leg1_intr_Value'] = np.maximum(dataFrame.strikes - list[0], 0)
        dataFrame['leg1_premium'] = np.repeat(list[1], dataFrame.__len__())
        if list[0] == 0.0:
            dataFrame['leg1Payoff'] = 0
        else:
            dataFrame['leg1Payoff'] = list[4] * (dataFrame['leg1_premium'] - dataFrame['leg1_intr_Value'])
    
        dataFrame['leg2_intr_Value'] = np.maximum(list[0] - dataFrame.strikes, 0)
...
def get_spot(soup):
    ...
    :return: string
    """
    return float(soup.find('span').text.split()[3])
...
def plot_payoff(payoff):
    ...
    ax1.set_xlim(spot-1000, spot+1000)
...
symbol = 'NIFTY'
soup = get_soup(symbol, expdate='27JUN2019')
req_row = get_req_row(soup)
spot = get_spot(soup)
...
if __name__ == '__main__':
    ...
    call_itm_strikes = get_itm_strikes(strike_list, option_type='call', spot=spot)
    call_otm_strikes = get_otm_strikes(strike_list, option_type='call', spot=spot)
    put_itm_strikes = get_itm_strikes(strike_list, option_type='put', spot=spot)
    put_otm_strikes = get_otm_strikes(strike_list, option_type='put', spot=spot)
