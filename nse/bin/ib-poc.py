...
tickerexch = input(
    "Enter the exchange (SMART, CBOE, CME, Default is CBOE): ") or 'CBOE'  # CBOE, SMART, get the exchange for routing
tickerprimaryexch = ''  # ''
tickercurr = 'USD'  # USD

interval = input(
    "Enter the interval (1 min, 2 mins, 5 mins, Default is 1 min): ") or '1 min'  # '1 min' get the interval for rounding
lookback = input(
    "Enter the lookback period (1 D, 9 D, 24 D, Default is 2 D): ") or '2 D'  # '24 D' get the lookback period
precision = int(input(
    "Enter the price precision in decimal places (0, 1, 2, Default is 1): ") or 1)  # '24 D' get the lookback period
...
def print_data_from_ib(msg):
    ...
    ticks.append(str(round(msg.close, precision)) + ' ' + datestamp(msg.date) + ' ' + code(
        msg.date))  # ticks will hold a list of tick information we need
    dates.append(datestamp(msg.date))  # get just the date in IB format
    prices.append(
        round(msg.close, precision))  # get the prices rounded off (Need to figure this out, maybe a user input)
