...
stockToPull = 'VEDL.NS','SBIN.NS','CAPF.NS'

def pullData(stock):
    try:
        print 'Currently trying to pull: ',stock
        print 'Current time is: ',str(datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'))
        ...
        try:
            ...
            print('File is recent. Not pulling data.')
        except Exception,e:
            print str(e)
            time.sleep(5)
            lastUnix = 0
